...
# === GEO & REGION PATTERNS ===
GEO_PATTERN = re.compile(r'(?<=\n|^)([A-Z]{2}|GCC|LATAM|ASIA|NORDICS)(?=\s|:|\n)', re.IGNORECASE)
...
def parse_message_block(block, sender, raw_message):
    geo_match = re.search(r'(?<=^|\n)([A-Z]{2}|GCC|LATAM|ASIA|NORDICS)', block, re.IGNORECASE)
...
def extract_deals_from_text(text, sender):
    deals = []
    blocks = re.split(r'(?=\n?[A-Z]{2}\s|\n?GCC|LATAM|ASIA|NORDICS)', text)
    for block in blocks:
        if any(code in block.upper() for code in ['CPA', '$', '%', 'FUNNEL', 'SOURCE']):
            deal = parse_message_block(block, sender, text)
            if deal['Geo']:
                deals.append(deal)
    return deals
